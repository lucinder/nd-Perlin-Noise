...
__global__ void perlin(float* noise) // TODO - Replace srand/rand with cuRAND
{
    ...
    for (int j = 0; j < N; j++) {
        coords[j] = ((float)rand()) / ((float)((unsigned)RAND_MAX + 1)); // fill with random values
    }
    ...
    srand(hash[i % (sizeof(hash) / sizeof(int))]); // reseed with pseudorandomness
    for (int j = 0; j < pow(2, N); j++) {
        int euclid_dist = 0;
        for (int k = 0; k < N; k++) {
            int idx = j * N + k;
            gradients[idx] = 0; // initialize to 0 for safety
            gradients[idx] = ((float)rand()) / ((float)((unsigned)RAND_MAX + 1)) * 2.0f - 1.0f; // load pseudorandom gradient
